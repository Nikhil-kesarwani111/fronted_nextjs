...
export default function PdfList() {
  ...
  const debouncedUpdatePdf = useCallback(
    debounce((pdf) => updatePdf(pdf), 500),
    []
  );
  ...
  function handlePdfChange(e, id) {
    ...
    const updated = { ...pdfs[idx], [name]: value };

    copy[idx] = updated;
    debouncedUpdatePdf(updated);
    setPdfs(copy);
  }
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.filters}>
        ...
        <button onClick={() => handleFilterChange(true)}>Selected</button>
        <button onClick={() => handleFilterChange(false)}>Not Selected</button>
      </div>
    </div>
  );
}
